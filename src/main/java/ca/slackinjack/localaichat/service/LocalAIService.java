...
public class LocalAIService {
    ...
    /*******************************************************
        There is almost nothing changed about this from
           the original file, which can be found at:
    https://github.com/TheoKanning/openai-java/blob/main/service/src/main/java/com/theokanning/openai/service/OpenAiService.java
    
            The only changes are the constructors.
    *******************************************************/
     ...
     */
    public LocalAIService(final String token, final String url) {
        this(token, url, DEFAULT_TIMEOUT);
    }
     ...
     */
    public LocalAIService(final String token, final String url, final Duration timeout) {
    ...
    /**
     * Creates a new OpenAiService that wraps OpenAiApi. Use this if you need
     * more customization, but use OpenAiService(api, executorService) if you
     * use streaming and want to shut down instantly
     *
     * @param api OpenAiApi instance to use for all methods
     */
    public LocalAIService(final OpenAiApi api, final String url) {
        this.api = api;
        this.executorService = null;
    }

    /**
     * Creates a new OpenAiService that wraps OpenAiApi. The ExecutorService
     * must be the one you get from the client you created the api with
     ...
     */
    public LocalAIService(final OpenAiApi api, final String url, final ExecutorService executorService) {
        this.api = api;
    ...
    /**
     * Calls the Open AI api, returns the response, and parses error messages if
     * the request fails
     */
    ...
    /**
     * Shuts down the OkHttp ExecutorService. The default behaviour of OkHttp's
     * ExecutorService (ConnectionPool) is to shut down after an idle timeout of
     * 60s. Call this method to shut down the ExecutorService immediately.
     */
    ...
    public OpenAiApi buildApi(String token, String url, Duration timeout) {
        ObjectMapper mapper = defaultObjectMapper();
        OkHttpClient client = defaultClient(token, timeout);
        Retrofit retrofit = defaultRetrofit(client, url, mapper);
    ...
    public static OkHttpClient defaultClient(String token, Duration timeout) {
        return new OkHttpClient.Builder()
                .addInterceptor(new LocalAIAuthenticationInterceptor(token))
                .connectionPool(new ConnectionPool(5, 1, TimeUnit.SECONDS))
                .readTimeout(timeout.toMillis(), TimeUnit.MILLISECONDS)
                .build();
    }

    public Retrofit defaultRetrofit(OkHttpClient client, String url, ObjectMapper mapper) {
    ...
    /**
     * Account information inquiry: including total amount and other
     * information.
     *
    ...
    /**
     * Account API consumption amount information inquiry. Up to 100 days of
     * inquiry.
     *
     * @param starDate
     * @param endDate
